...
// Z字形打印二叉树
void treePrintZ(BinaryTreeNode *pRoot)
{
    if (pRoot == nullptr)
        return;
    
    stack<BinaryTreeNode *> cached{};
    stack<BinaryTreeNode *> currentLayer{};
    // 是否从左到右
    bool flag = true;
    currentLayer.push(pRoot);
    
    while (!currentLayer.empty()) {
        BinaryTreeNode *node = currentLayer.top();
        ...
        {
            printf("\n");
            stack<BinaryTreeNode *> temp = currentLayer;
            currentLayer = cached;
            cached = temp;
            flag = !flag;
        }
